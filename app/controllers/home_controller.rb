class HomeController < ApplicationController
    ...
    def feedAlbum
        @albums = Album.where(shared:true, user_id: current_user.followings.ids + [current_user.id]).order(updated_at: :desc).page(params[:page]).per(PER_PAGE)
        render template: "feed/albums"
    end

    def feedPhoto
        @photos = Photo.where(shared:true, album_id:nil,user_id: current_user.followings.ids + [current_user.id]).order(updated_at: :desc).page(params[:page]).per(PER_PAGE)
